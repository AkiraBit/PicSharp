...
function SettingsCompressionTinyPngApiKeys() {
  const t = useI18n();
  const { compression_tinypng_api_keys: tinypngApiKeys, set } =
    useSettingsStore(
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="cursor-pointer min-w-[50px]">
      ...
      cell: ({ row }) => (
        <Button
          ...
          onClick={() => {
            showAlertDialog({
              title: t(
                "settings.compression.tinypng_api_keys.table.delete_title"
              ),
              description: t(
    ...
    if (isValidArray(tinypngApiKeys)) {
      ...
      const newApiKeys = await Promise.all(
        tinypngApiKeys.map(async (item) => {
          const { ok, compressionCount = "-" } = await Tinify.validate(
            item.api_key
          );
          return {
            ...item,
...
const nameValidationStrategy: ValidationStrategy = {
  async validate(data, tinypngApiKeys) {
    const exists =
      tinypngApiKeys.findIndex((item) => item.name === data.name) !== -1;
    return {
      isValid: !exists,
      error: exists
        ? {
            field: "name",
            msgKey:
              "settings.compression.tinypng_api_keys.form.name_already_exists",
          }
        : undefined,
    };
  },
...
const apiKeyValidationStrategy: ValidationStrategy = {
  async validate(data, tinypngApiKeys) {
    const exists =
      tinypngApiKeys.findIndex((item) => item.api_key === data.api_key) !== -1;
    return {
      isValid: !exists,
      error: exists
        ? {
            field: "api_key",
            msgKey:
              "settings.compression.tinypng_api_keys.form.api_already_exists",
          }
        : undefined,
    };
  },
    ...
    return {
      ...
      error: !ok
        ? {
            field: "api_key",
            msgKey:
              "settings.compression.tinypng_api_keys.form.invalid_api_key",
          }
        : undefined,
      payload: {
...
function AddApiKeyDialog() {
  ...
  const handleSubmit = async (data: TinypngApiKeyFormData) => {
    ...
    await set(SettingsKey.compression_tinypng_api_keys, [
      ...tinypngApiKeys,
      {
        ...data,
        usage:
          Number(validationResult?.payload?.api_key_validity?.usage || 0) ||
          "-",
        status: "valid",
        created_at: Date.now(),
      },
  ...
  const ErrorTableColumns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="text-nowrap">{row.original.err_msg}</div>
      ),
    },
  ];
            ...
            if (data.success) {
              const validateResults = await Promise.all(
                data.data.map(async (item) => {
                  ...
                  return {
                    ...result,
                    ...item,
                    err_msg: result.error?.msgKey
                      ? t(result.error.msgKey as any)
                      : "",
                  };
                })
              );
              ...
              await set(
                ...
                data.data.map((item) => ({
                  ...
                  usage:
                    Number(
                      validateResults.find((i) => i.api_key === item.api_key)
                        ?.payload?.api_key_validity?.usage || 0
                    ) || "-",
                  status: "valid",
                  created_at: Date.now(),
                }))
      ...
      <DialogContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)}>
            <DialogHeader>
              <DialogTitle>
                {t("settings.compression.tinypng_api_keys.form.add_title")}
              </DialogTitle>
              <DialogDescription>
                {t(
                  "settings.compression.tinypng_api_keys.form.add_description"
                )}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="flex flex-col gap-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t("settings.compression.tinypng_api_keys.form.name")}
                      </FormLabel>
                      <FormControl>
                        <Input
                          type="text"
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        {t(
                          "settings.compression.tinypng_api_keys.form.api_key"
                        )}
                      </FormLabel>
                      <FormControl>
                        <Input
